...
export async function getExchangeRate(from = 'PHP', to = 'USD'): Promise<number> {
  ...
  if (rateCache[cacheKey] && now - cacheTimestamp < config.cache.exchangeRateDuration) {
    return rateCache[cacheKey];
  }
  
  try {
    // Get API key directly from config
    const apiKey = config.keys.exchangeRate;
    
    // Use the pair conversion endpoint for better performance
    const response = await fetch(`${config.urls.exchangeRateApi}/${apiKey}/pair/${from}/${to}`);
...
async function getExchangeRateFromStandardEndpoint(from: string, to: string): Promise<number> {
  ...
  try {
    ...
    const response = await fetch(`${config.urls.exchangeRateApi}/${apiKey}/latest/${from}`);
